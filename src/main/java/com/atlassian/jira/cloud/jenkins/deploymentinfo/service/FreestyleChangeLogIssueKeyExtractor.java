...
public class FreestyleChangeLogIssueKeyExtractor implements FreestyleIssueKeyExtractor {
    ...
    public Set<String> extractIssueKeys(final AbstractBuild<?, ?> freestyleBuild) {
        ...
        final List<ChangeLogSet<? extends ChangeLogSet.Entry>> changeSets = freestyleBuild.getChangeSets();
        for (ChangeLogSet<? extends ChangeLogSet.Entry> changeSet : changeSets) {
            ...
            for (Object item : changeSetEntries) {
                ...
                if (changeSetEntry instanceof GitChangeSet) {
                    allIssueKeys.addAll(
                            IssueKeyStringExtractor.extractIssueKeys(((GitChangeSet) changeSetEntry).getComment()));
                }
        ...
        return allIssueKeys.stream().limit(ISSUE_KEY_MAX_LIMIT).map(IssueKey::toString).collect(Collectors.toSet());
    }
}
